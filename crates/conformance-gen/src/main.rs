...
use json_schema_transformer::emit::{
    Emitter, KotlinEmitter, PydanticEmitter, SwiftEmitter, ZodEmitter,
};
use serde::Serialize;
...
fn emit_guarded(
    ...
    let result = catch_unwind(AssertUnwindSafe(|| {
        let converted = Converter::convert(schema).map_err(|e| e.to_string())?;
    ...
    for file in &suite_files {
        ...
        for (gi, group) in suite_groups.into_iter().enumerate() {
            ...
            for (lang, emitter) in &langs {
                match emit_guarded(emitter.as_ref(), &group.schema, &name) {
                    Ok(code) => {
                        let rel = match *lang {
