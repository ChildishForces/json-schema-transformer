...
impl Emitter for SwiftEmitter {
    ...
    fn emit_with_options(
        ...
        // Shared-helpers mode: all utility helpers (ValidationError, format
        ...
        // companion helpers file (see [`Emitter::helpers_content`]) and
    ...
    fn helpers_content(&self) -> Option<String> {
        ...
        // Full ValidationError with every case any generated file may throw
        s.push_str(VALIDATION_ERROR_FULL);
        s.push('\n');
        ...
        for helper in [
            ...
            IP_ADDRESS_HELPER,
            TIME_STRING_HELPER,
            DURATION_STRING_HELPER,
            BASE64_STRING_HELPER,
        ] {
            s.push_str(helper);
            s.push('\n');
        }
...
/// The complete ValidationError enum for the shared helpers file (inline mode
/// emits a per-file `private` enum with only the cases that file needs).
const VALIDATION_ERROR_FULL: &str = r#"enum ValidationError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .invalidEmail(let v): "Invalid email address: \(v)"
        case .invalidHostname(let v): "Invalid hostname: \(v)"
        case .invalidIPv4(let v): "Invalid IPv4 address: \(v)"
        case .invalidIPv6(let v): "Invalid IPv6 address: \(v)"
        case .invalidIP(let v): "Invalid IP address: \(v)"
        case .invalidTime(let v): "Invalid time: \(v)"
        case .invalidDuration(let v): "Invalid duration: \(v)"
        case .invalidBase64(let v): "Invalid base64: \(v)"
        case .outOfRange(let field, let detail): "\(field): \(detail)"
        }
    }
}
"#;
...
fn emit_wrapper_struct(name: &str, ir: &SchemaIr, defs: &[DefEntry]) -> String {
    ...
    match inner {
        ...
        SchemaIr::Object(_) => emit_struct(name, inner, defs),
