...
use super::{EmitOptions, Emitter, HelperSet, module_header, needs_quoting};
...
impl Emitter for ZodEmitter {
    ...
    fn emit_collecting(
        ...
        (emit_module(converted, name, options), detect_helpers(converted))
    }
    ...
    fn helpers_content_for(&self, needs: &HelperSet) -> Option<String> {
        ...
        if needs.contains("stable_stringify") {
            s.push_str(&STABLE_STRINGIFY_HELPER.replacen(
                "function _stableStringify",
                "export function _stableStringify",
                1,
            ));
            s.push_str("\n\n");
        }
        if needs.contains("deep_equal") {
            s.push_str(
                &DEEP_EQUAL_HELPER.replacen("function _deepEqual", "export function _deepEqual", 1),
            );
            s.push_str("\n\n");
        }
        if needs.contains("jsi") {
            s.push_str(
                &JSI_HELPER.replacen("function _jsiValidate", "export function _jsiValidate", 1),
            );
        }
    ...
    fn helpers_content(&self) -> Option<String> {
        ...
        all.insert("stable_stringify");
        all.insert("deep_equal");
        all.insert("jsi");
...
/// Helper components a module for this schema references. Keys:
/// "stable_stringify", "deep_equal", "jsi" (no interdependencies).
fn detect_helpers(converted: &ConvertedSchema) -> HelperSet {
    let mut needs = HelperSet::default();
    if needs_deep_equal(&converted.root) || converted.defs.iter().any(|d| needs_deep_equal(&d.schema)) {
        needs.insert("deep_equal");
    }
    if has_unique_items(&converted.root) || converted.defs.iter().any(|d| has_unique_items(&d.schema)) {
        needs.insert("stable_stringify");
    }
    if matches!(converted.root, SchemaIr::Interpreted { .. }) {
        needs.insert("jsi");
    }
    needs
}

const STABLE_STRINGIFY_HELPER: &str = include_str!("stable_stringify.ts");

const DEEP_EQUAL_HELPER: &str = include_str!("deep_equal.ts");
...
const JSI_HELPER: &str = include_str!("jsi_validator.ts");
...
pub fn emit_module(
    ...
    let needs = detect_helpers(converted);

    if let Some(shared) = &options.helpers {
        // Collection mode: reference the companion module instead of inlining
        let mut names: Vec<&str> = Vec::new();
        if needs.contains("stable_stringify") {
            names.push("_stableStringify");
        }
        if needs.contains("deep_equal") {
            names.push("_deepEqual");
        }
        if needs.contains("jsi") {
            names.push("_jsiValidate");
        }
        if !names.is_empty() {
            let stem = shared.file_name.trim_end_matches(".ts");
            let import_path = if shared.dir_prefix.is_empty() {
                format!("./{stem}")
            } else {
                format!("{}{stem}", shared.dir_prefix)
            };
            out.push_str(&format!(
                "import {{ {} }} from \"{}\";\n",
                names.join(", "),
...
pub fn zod_for(ir: &SchemaIr, defs: &[DefEntry]) -> String {
    match ir {
        SchemaIr::Interpreted { schema, remotes } => {
            // Embed via JSON.parse: a raw JS object literal would treat a
            // "__proto__" key as the prototype setter and silently drop it
            ...
            return format!(
                "z.any().superRefine((val, ctx) => {{ if (!_jsiValidate(JSON.parse({schema_literal}), JSON.parse({remotes_literal}) as Record<string, unknown>, val)) ctx.addIssue({{ code: z.ZodIssueCode.custom, message: \"does not match schema\" }}); }})"
            );
        }
        SchemaIr::String(c) => {
            ...
            if c.min_length.is_some() || c.max_length.is_some() {
                ...
                if let Some(min) = c.min_length {
                    checks.push_str(&format!(" if (n < {min}) ctx.addIssue({{ code: z.ZodIssueCode.custom, message: \"must contain at least {min} character(s)\" }});"));
                }
                if let Some(max) = c.max_length {
                    checks.push_str(&format!(" if (n > {max}) ctx.addIssue({{ code: z.ZodIssueCode.custom, message: \"must contain at most {max} character(s)\" }});"));
                }
                s.push_str(&format!(
                    ".superRefine((v, ctx) => {{ const n = Array.from(v).length;{checks} }})"
                ));
            }
            s
        }
...
fn zod_ts_type(ir: &SchemaIr, defs: &[DefEntry]) -> String {
    match ir {
        ...
        SchemaIr::ConstEqual(_) | SchemaIr::ComplexEnum(_) | SchemaIr::Interpreted { .. } => "any".to_string(),
...
fn number_constraints(c: &NumberConstraints) -> String {
    ...
    if let Some(mul) = c.multiple_of {
        // Division-based check avoids IEEE 754 remainder artifacts
        // (e.g. any integer is a multiple of 1e-8)
        s.push_str(&format!(
            ".superRefine((v, ctx) => {{ const q = v / {mul}; if (!Number.isFinite(q) || q !== Math.round(q)) ctx.addIssue({{ code: z.ZodIssueCode.custom, message: \"must be a multiple of {mul}\" }}); }})"
        ));
    }
    s
}
