...
impl Emitter for ZodEmitter {
    ...
    fn helpers_content(&self) -> Option<String> {
        ...
        s.push_str("\n\n");
        s.push_str(&DEEP_EQUAL_HELPER.replacen("function _deepEqual", "export function _deepEqual", 1));
        s.push_str("\n\n");
        s.push_str(&JSI_HELPER.replacen("function _jsiValidate", "export function _jsiValidate", 1));
        s.push('\n');
        Some(s)
    }
...
pub fn emit_module(
    ...
    let ir_needs_interpreter = matches!(converted.root, SchemaIr::Interpreted { .. });

    if let Some(helpers_file) = &options.helpers_file {
        // Shared-helpers mode: reference the companion module instead of inlining
        ...
        if ir_needs_interpreter {
            names.push("_jsiValidate");
        }
        if !names.is_empty() {
            let import_path = helpers_file.trim_end_matches(".ts");
            out.push_str(&format!(
                "import {{ {} }} from \"./{}\";\n",
                names.join(", "),
                import_path
        ...
        if ir_needs_deep_equal || ir_needs_stable_stringify || ir_needs_interpreter {
            out.push_str("\n// --- Helpers ---\n");
        }
        if ir_needs_stable_stringify {
