...
console.log(
  `zod conformance: ${results.pass}/${results.total} passed, ${results.fail} failed (${pct.toFixed(2)}%) in ${(elapsed / 1000).toFixed(2)}s`
);
...
if (worst.length > 0) {
  ...
  for (const k of worst) {
    console.log(
      `  ${k.keyword.padEnd(24)} ${String(k.fail).padStart(4)} failed / ${k.total} (${((k.pass / k.total) * 100).toFixed(1)}% pass)`
    );
  }
} else {
  console.log('\nNo failing keywords.');
